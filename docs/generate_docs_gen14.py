# ============================================================
# VERSION OVERVIEW — V14 (relative to V13)
# ============================================================
#
# This version refines page generation consistency and footer handling.
# It does not change how resources are submitted (YAML + pages.xlsx),
# but improves how generated pages are finalized and cleaned.
#
# ------------------------------------------------------------
# 6) Standardized EU funding footer injection
# ------------------------------------------------------------
# V14 introduces a robust, generator-level mechanism to ensure that the
# EU funding acknowledgment is:
#   - always present,
#   - always rendered exactly once,
#   - and always placed at the very end of the page.
#
# Any previously existing EU footer variants (HTML blocks, tables,
# horizontal rules, or duplicated footers) are removed before insertion.
# This guarantees consistent rendering across regenerated pages and
# prevents footer duplication when re-running the generator.
#
# ------------------------------------------------------------
# 7) Removal of unintended trailing separators
# ------------------------------------------------------------
# Earlier versions could introduce extra horizontal rules ("---")
# at the end of pages due to per-resource separators.
#
# In V14:
#   - Resource separators are inserted only *between* resource blocks,
#   - never after the final resource,
#   - ensuring the EU footer is not preceded by an unintended divider.
#
# ------------------------------------------------------------
# 8) Deterministic regeneration without scrubbing
# ------------------------------------------------------------
# Since pages are always fully regenerated, V14 relies on deterministic
# output construction rather than external scrubbers or manual cleanup.
#
# Footer cleanup and reinsertion are handled internally during generation,
# making repeated runs safe, idempotent, and predictable.
#
# ------------------------------------------------------------
# Summary
# ------------------------------------------------------------
# V14 focuses on output hygiene and reproducibility:
#   - a single authoritative EU funding footer,
#   - no visual artifacts at page endings,
#   - and clean regeneration without manual intervention.
#
# ============================================================
...
EU_BLOCK_REMOVE_RE = re.compile(
    ...
    (?:\s*<hr\b[^>]*>\s*)?
    \s*
    (?:<!--\s*EU_FUNDING_FOOTER\s*-->.*?)(?=\Z)
    |
    (?:\n{0,2}(?:---\s*\n{0,2})?)?
    (?:\s*<hr\b[^>]*>\s*)?
    \s*
    <table\b.*?co-funded.*?</table>\s*(?=\Z)
    |
    (?:\n{0,2}(?:---\s*\n{0,2})?)?
    (?:\s*<hr\b[^>]*>\s*)?
    \s*
    <div\b.*?co-funded.*?</div>\s*(?=\Z)
    """,
    flags=re.IGNORECASE | re.DOTALL | re.VERBOSE,
)

def ensure_eu_block_at_end(md_body: str) -> str:
    body = (md_body or "").rstrip()
    ...
    for _, row in df.iterrows():
        ...
        if resources_for_topic:
            ...
            for idx, res in enumerate(resources_for_topic, start=1):
                ...
                if idx < len(resources_for_topic):
                    resources_list_md += "\n---\n\n"
        ...
        # Ensure EU acknowledgment is always at the very bottom
        final_body = ensure_eu_block_at_end(final_body)
